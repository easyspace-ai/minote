...
**Runtime note (Minote / LangGraph / `ask_clarification`):** The steps below say `AskUserQuestion` — that is a **Claude Code** UI affordance. If your agent only has **`ask_clarification`**, use that tool instead with valid JSON: `question` (string), optional `options` with `id` / `label` / `value`, optional `type` (`choice`, `text`, `confirm`), and **`required` as JSON booleans `true` or `false`** — never strings, and never paste error text or prose into `required`. Combine the discovery questions into one `question` text and/or one `choice` with options when the UI supports a single interrupt; otherwise ask in separate turns.

**Ask ALL questions in a single AskUserQuestion call** (or one equivalent `ask_clarification` as above) so the user fills everything out at once:
...
5. **Confirm via AskUserQuestion** or **`ask_clarification`** (header: "Outline"): "Does this slide outline and image selection look right?" Options: Looks good / Adjust images / Adjust outline
