...
### 9. Service 依赖 HTTP 上下文

```go
// ❌ 禁止
func (s *userService) Create(w http.ResponseWriter, r *http.Request, req Request) error {
    // 禁止传入 http.ResponseWriter 和 *http.Request
}
...
func (h *UserHandler) Create(w http.ResponseWriter, r *http.Request) {
    if user.Age < 18 {
        response.BadRequest(w, errors.New("must be 18+"))  // 业务逻辑应在 Service
        return
    }
}
