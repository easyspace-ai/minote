# AI 后端开发与测试规范（Go 1.25+ + Eino）

> 本规范用于指导 **AI 与开发人员** 在 YouMind 后端项目中进行一致、可验证、可自动化的开发与测试工作。
>
...
```text
internal/
├── handler/              # HTTP Handler (标准库 net/http)
│   ├── xxx.go
│   └── xxx_test.go       # 接口测试
├── service/              # 业务逻辑
│   ├── xxx_service.go
...
func TestGetUserAPI(t *testing.T) {
    ...
    mux.HandleFunc("/users/{id}", GetUserHandler(mockService))
